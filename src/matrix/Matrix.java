    ...
    public int getLocalMinimumCount() {
        ...
        for (int i = 1; i < vectors.length - 1; i++) {
            for (int j = 1; j < vectors[0].size() - 1; j++) {
                ...
                float leftUpValue = getValue(i - 1, j - 1);
                float middleUpValue = getValue(i - 1, j);
                float rightUpValue = getValue(i - 1, j + 1);
                float leftMiddleValue = getValue(i, j - 1);
                float rightMiddleValue = getValue(i, j + 1);
                float leftDownValue = getValue(i + 1, j - 1);
                float middleDownValue = getValue(i + 1, j);
                float rightDownValue = getValue(i + 1, j + 1);

                if (current < leftUpValue &&
                        current < middleUpValue &&
                        current < rightUpValue &&
                        current < leftMiddleValue &&
                        current < rightMiddleValue &&
                        current < leftDownValue &&
                        current < middleDownValue &&
                        current < rightDownValue)
                    ++localMinimumCount;
            }
        }
